...
def skills_info(city = None, state = None):
    ...
    for i in range(1,int(num_pages+1)): # Loop through all of our search result pages
        ...
        job_URL = [str(base_url) + str(link.get('href')) for link in job_link_area.find_all('a')] # Get the URLS for the jobs
        job_URLZ = list(filter(lambda x:'clk' in x, job_URL)) # Now get just the job related URLS
        #print(job_URLZ)    
        job_URLS.extend(job_URLZ)
        #print(job_URLS)
    ...
    #final_frame = pd.DataFrame.from_dict(overall_total_skills.items(), columns = ['Term', 'NumPostings']) # Convert these terms to a 
                                                                                                # dataframe 
